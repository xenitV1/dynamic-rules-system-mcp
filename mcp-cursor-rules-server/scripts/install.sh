...
REPO_ZIP_URL=${1:-"https://github.com/xenitV1/cursor-dynamic-rules-system-mcp/archive/refs/heads/main.zip"}
SUBDIR=${2:-"mcp-cursor-rules-server"}
...
BIN="$HOME/.local/bin"
mkdir -p "$BIN"
SHIM="$BIN/mcp-cursor-rules-server"
cat > "$SHIM" <<'EOF'
#!/usr/bin/env bash
...
echo "Ensure $BIN is on PATH. Then run: mcp-cursor-rules-server"
