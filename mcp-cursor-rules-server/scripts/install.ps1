param(
  [string]$RepoZipUrl = "https://github.com/xenitV1/cursor-dynamic-rules-system-mcp/archive/refs/heads/main.zip",
  [string]$SubDir = "mcp-cursor-rules-server",
...
$shim = Join-Path $bin 'mcp-cursor-rules-server.cmd'
@"
@echo off
...
if ($envPath -notlike "*${bin}*") {
  ...
  Write-Host "Added $bin to PATH (User). Restart terminal to use 'mcp-cursor-rules-server'." -ForegroundColor Yellow
}

Write-Host "Done. Try: mcp-cursor-rules-server" -ForegroundColor Green
